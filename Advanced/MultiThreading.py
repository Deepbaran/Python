...
"""
We can create a thread with threading.Thread().
...
Call thread.join() to tell the program that it should wait for this thread to complete before it continues woth the rest of the code.
"""
...
def increase():
    global database_value # need to modify the global value

    # get a local copy (simulate data retreiving)
    ...
    # write the calculated new value into tje global variable
...
# In our example the critical code section where database values are retrieved and modified is now locked. This prevents the second thread from modyfing the global data at the same time.
...
After lock.acquire() you should never forget to call lock.release() to unblock the code. You can also use a lock as a context manager, which will safely lock and unlock your code. It is recommended to use a lock this way. 
"""
...
- q.get() : remove and return the first item. By default, it blocks till the item is available.
...
def worker(q, lock):
    while True:
        value = q.get() # block until the item is available
        ...
        # For each get(), subsequent call to task_done() tells the queue
